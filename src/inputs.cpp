...
struct ScreenShot{
    ...
    void operator() (cv::Mat& cv_img) {
        ...
        cvtColor(cv_img, cv_img, COLOR_RGBA2BGR);
    }
...
int input_screen(frame_chan_t &to_finger, frame_chan_t &to_gesture) {
    ScreenShot cap();
    broadcast_size.push(capture_size_t { scap.width, scap.height });
    Mat frame;
    int i = 0;
    while (true) {
        cap(frame);
...
int input_camera(frame_chan_t &to_finger, frame_chan_t &to_gesture) {            
...
int input_white(frame_chan_t &to_finger, frame_chan_t &to_gesture) {
...
int input(frame_chan_t &to_finger, frame_chan_t &to_gesture, cap_size_chan_t &broadcast_size, capture_source_t source) {
    switch (source) {
        ...
        case WHITEBOARD:
            return input_inner(to_finger, to_gesture, white_frame);
    }
}